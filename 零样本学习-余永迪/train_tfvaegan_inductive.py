...
if opt.cuda:
    torch.cuda.manual_seed_all(opt.manualSeed)  # manual_seed_all
...
def loss_fn(recon_x, x, mean, log_var):
    ...
    BCE = torch.true_divide(BCE.sum(), x.size(0))  # BCE.sum()/x.size(0)
    KLD = -0.5 * torch.true_divide(torch.sum(1 + log_var - mean.pow(2) - log_var.exp()),
                                   x.size(0))  # torch.sum(1 + log_var - mean.pow(2) - log_var.exp())/x.size(0)
    return BCE + KLD
...
def WeightedL1(pred, gt):
    ...
    loss = wt * (pred - gt).abs()
    return torch.true_divide(loss.sum(), loss.size(0))  # return loss.sum()/loss.size(0)
...
def generate_syn_feature(netG, classes, attribute, num, netF=None, netDec=None):
    ...
    for i in range(nclass):
        ...
        syn_noisev = Variable(syn_noise, requires_grad=False)  # volatile=True
        syn_attv = Variable(syn_att, requires_grad=False)  # volatile=True
        output = feedback_module(gen_out=syn_noisev, att=syn_attv, feed_weight=opt.a2, netG=netG, netDec=netDec,
                                 netF=netF)
...
for epoch in range(0, opt.nepoch):
    ...
    for loop in range(0, opt.feedback_loop):
        for i in range(0, data.ntrain, opt.batch_size):
            ...
            for iter_d in range(opt.critic_iter):
                ...
                # feedback loop
                if loop == 1:
                    fake = feedback_module(gen_out=latent_code, att=input_attv, feed_weight=opt.a1, netG=netG,
                                           netDec=netDec, netF=netF)
                else:
            ...
            if opt.recons_weight > 0 and opt.freeze_dec:  # and opt.freeze_dec
                for p in netDec.parameters():
                    p.requires_grad = False
            ...
            latent_code = eps * std + means
            if loop == 1:
                recon_x = feedback_module(gen_out=latent_code, att=input_attv, feed_weight=opt.a1, netG=netG,
                                          netDec=netDec, netF=netF)
            else:
            ...
            if opt.encoded_noise:
                ...
                if loop == 1:
                    fake = feedback_module(gen_out=latent_code_noise, att=input_attv, feed_weight=opt.a1, netG=netG,
                                           netDec=netDec, netF=netF)
                else:
            ...
            errG += (opt.recons_weight * R_cost + opt.cross_weight * cross_loss)
            errG.backward()
            optimizerE.step()
            optimizerG.step()
            if loop == 1:
    ...
    # writer.add_scalar("loss", running_loss, val_accurate)
    # writer.add_graph(netE)
    # writer.close()
    ...
    zsl_cls = classifier.CLASSIFIER(syn_feature, util.map_label(syn_label, data.unseenclasses), data,
...
writer = SummaryWriter("loss")
writer.add_scalar("loss", D_cost.item(), G_cost.item())
# writer.add_graph(model,input_res,input_att)
writer.close()
if opt.gzsl:
