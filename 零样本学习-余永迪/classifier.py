...
class CLASSIFIER:
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            ...
            H = torch.true_divide(2*acc_seen*acc_unseen, (acc_seen+acc_unseen))  # 2*acc_seen*acc_unseen / (acc_seen+acc_unseen)
            if H > best_H:
                best_seen = acc_seen
    ...
    def val_gzsl(self, test_X, test_label, target_classes): 
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                inputX = Variable(test_X[start:end].cuda(), requires_grad=False)  # volatile=True
            else:
                inputX = Variable(test_X[start:end], requires_grad=False)  # volatile=True
    ...
    def compute_per_class_acc_gzsl(self, test_label, predicted_label, target_classes):
        ...
        for i in target_classes:
            idx = (test_label == i)
            acc_per_class += torch.true_divide(torch.sum(test_label[idx]==predicted_label[idx]), torch.sum(idx))  # torch.sum(test_label[idx]==predicted_label[idx]) / torch.sum(idx)
    ...
    def compute_per_class_acc(self, test_label, predicted_label, nclass):
        ...
        for i in range(nclass):
            idx = (test_label == i)
            acc_per_class[i] = torch.true_divide(torch.sum(test_label[idx]==predicted_label[idx]), torch.sum(idx)) # torch.sum(test_label[idx]==predicted_label[idx]) / torch.sum(idx)
